...
{
    public partial class NuevoVerCategorias : Form
    {
        ...
        */
        ...
        private void NuevoVerCategorias_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        */
        ...
        {
            if (txtnombrecategoria.Text.Length == 0 || txtdescripcioncategoria.Text.Length == 0)
            {
                // CREANDO MENSAJE EN VENTANA FLOTANTE PERSONALIZADO
                if (MessageBox.Show("Lo sentimos pero ha ocurrido un error, tome en cuenta lo siguiente:\n- No puede dejar campos vacíos.\n- Por favor sea cuidadoso a la hora de agregar una nueva categoría de productos, respetando las métricas establecidas por la empresa.", "Error al Intentar Agregar Registro",
            ...
            else
            {
                ...
                string query = "INSERT INTO CategoriaProductos (Nombre_Categoria,Descripcion_Categoria) VALUES (@nombrecategoriaP,@descripcioncategoriaP)";
                ...
                // A LA BASE DE DATOS...
                comando.Parameters.AddWithValue("@nombrecategoriaP", txtnombrecategoria.Text);              // NOMBRE DE CATEGORIA
                comando.Parameters.AddWithValue("@descripcioncategoriaP", txtdescripcioncategoria.Text);    // DESCRIPCION COMPLETA DE CATEGORIA
                comando.ExecuteNonQuery(); // EJECUTANDO COMANDO Y ENVIANDO DATOS HACIA LA BASE DE DATOS
                // CREANDO MENSAJE EN VENTANA FLOTANTE PERSONALIZADO
                if (MessageBox.Show("Nueva categoría de producto agregada con éxito!.", "Registrando Nueva Categoría de Producto",
                 ...
                 * REALIZADO EL NUEVO REGISTRO A LA BASE DE DATOS {CONSTRUCTORES}
                 -----------------------------------------------------------------------------------------*/
                txtnombrecategoria.Text = " "; txtdescripcioncategoria.Text = " ";
                con.Close();// AL FINALIZAR CADA NUEVO REGISTRO, SE PROCEDE A CERRAR LA CONEXION A ESPERA DE UNA NUEVA PETICION
            }
        }
    }
