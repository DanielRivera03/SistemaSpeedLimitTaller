...
namespace SpeedLimitTaller
{
    public partial class ModificarCategorias : Form
    {
        ...
        */
        ...
        private void ModificarCategorias_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        */
        ...
        /*
            --> BOTON AGREGAR NUEVAS CATEGORIAS 
        */
        ...
        private void ModificarRegistroCategorias_Click(object sender, EventArgs e)
        {
            if (txtidcategoria.Text.Length == 0 || txtnombrecategoria.Text.Length == 0 || txtdescripcioncategoria.Text.Length == 0)
            {
                ...
                if (MessageBox.Show("Lo sentimos pero ha ocurrido un error, tome en cuenta lo siguiente:\n- No puede dejar campos vacíos.\n- Por favor sea cuidadoso a la hora de modificar una categoría de producto, respetando las métricas establecidas por la empresa.", "Error al Intentar Agregar Registro",
                    MessageBoxButtons.OK, MessageBoxIcon.Error) == DialogResult.OK) ;
                con.Close();// AL FINALIZAR CADA NUEVO REGISTRO, SE PROCEDE A CERRAR LA CONEXION A ESPERA DE UNA NUEVA PETICION
            } // SI HA SELECCIONADO UN REGISTRO, ENTONCES...
            else
            {
                // CREANDO VARIABLE TIPO QUERY QUE ALMACENA LA CADENA DE NUEVA MODIFICACION CON SU CONDICION
                string query = "UPDATE CategoriaProductos SET Nombre_Categoria = @nombrecategoriaM, Descripcion_Categoria = @descripcioncategoriaM WHERE ID_categoria=@ID_categoria";
                // --> WHERE ID_categoria=@ID_categoria" IMPORTANTE: SI NO SE REFERENCIA POR DEFECTO NO MUESTRA ERROR PERO SE PRODUCE UN ERROR LOGICO, AL ACTUALIZAR ABSOLUTAMENTE
                ...
                comando.Parameters.AddWithValue("@nombrecategoriaM", txtnombrecategoria.Text);              // NOMBRE DE CATEGORIA
                comando.Parameters.AddWithValue("@descripcioncategoriaM", txtdescripcioncategoria.Text);    // DESCRIPCION DE CATEGORIA
                comando.Parameters.AddWithValue("@ID_categoria", txtidcategoria.Text);                      // REFERENCIANDO ID CATEGORIA A EDITAR EN EL REGISTRO
                comando.ExecuteNonQuery(); // EJECUTANDO COMANDO Y ENVIANDO DATOS HACIA LA BASE DE DATOS
                // CREANDO MENSAJE EN VENTANA FLOTANTE PERSONALIZADO
                if (MessageBox.Show("Categoría de Producto Modificada Con Exito", "Modificando Categoría de Producto Existente",
        ...
        /*
            --> BOTON SELECCIONAR CATEGORIAS A EDITAR EN EL REGISTRO 
        */
